...
export class RatingController {
  ...
  @Get('/:ratingId')
  @ApiOperation({ summary: 'Retornar uma avaliação' })
  @ApiParam({
    name: 'ratingId',
    required: true,
  ...
  @ApiResponse({
    status: 200,
    description: 'A avaliação foi retornada com sucesso.',
    type: [RatingResponseDto],
  })
  @ApiResponse({ status: 404, description: 'A avaliação não foi encontrado.' })
  @ApiResponse({
    status: 500,
    description: 'Houve um erro ao procurar pela avaliação.',
  })
  async getRating(
    ...
    try {
      ...
      if (!rating) {
        return response.status(HttpStatus.NO_CONTENT).json({
          message: 'Avaliação não encontrada para este usuário.',
        });
      }

      return response.status(HttpStatus.OK).json({
        message: 'Avaliação retornada com sucesso.',
        rating,
      });
    } catch (error) {
      return response.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
  ...
  @ApiResponse({
    status: 204,
    description: 'Não há avaliações para esse usuário.',
  })
  ...
  async getUserSubmittedRatings(
    ...
    try {
      const ratings = await this.ratingService.getUserSubmittedRatings(userId);
  ...
  async removeDriverRating(
    ...
    try {
      const removedRating =
        await this.ratingService.removeDriverRating(ratingId);
  ...
  async removeMemberRating(
    ...
    try {
      const removedRating =
        await this.ratingService.removeMemberRating(ratingId);
